...
function createAnimation (recordedData, morphTargetDictionary, bodyPart) {
  ...
  if (!recordedData || !Array.isArray(recordedData) || recordedData.length === 0) {
    console.warn("createAnimation: recordedData is empty or invalid", recordedData);
    return null;
  }

  if (!morphTargetDictionary) {
    console.warn("createAnimation: morphTargetDictionary is invalid", morphTargetDictionary);
    return null;
  }
